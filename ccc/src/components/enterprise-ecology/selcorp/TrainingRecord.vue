<template>
  <div class="page page-bill page-bill-max">
    <CorpTabs tabvalue="pxjl" :corpId="corpId"></CorpTabs>
      ...
      <div class="baseinfo">
        ...
        >
          <table cellspacing="0" cellpadding="0">
            ...
            <tr>
              <td colspan="2">
                <FormItem label="中国系统生态伙伴培训记录" prop>
...
export default {
  ...
  methods: {
    ...
    getInfo() {
      var that = this;
      this.loading = 1;
      this.$http.post('/api/engine/enterprise-ecology/corp/getFile', { "corpId": this.corpId, "pageType": "trainingRecord" }).then((res) => {
        this.loading = 0;
        if (res.data.code === 0) {
    ...
    saveItem() {
      ...
      param.pageType = 'trainingRecord';
      param.fileIds = [];
      for (var i = 0; i < that.formItem.files.split(',').length; i++) {
        param.fileIds.push({ 'fileId': that.formItem.files.split(',')[i], 'catalog': 'pxjl' });
      }
      this.loading = 1;
