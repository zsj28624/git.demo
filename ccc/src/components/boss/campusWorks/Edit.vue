            ...
            <tr>
              <td colspan="3">
                <!-- <FormItem label="公司全称" prop="name">
                  <Select v-model="formItem.name" style="width:100%;" filterable @on-change="changeName"  ref="setSelect">
                    <Option v-for="item in selectProject" :value="item.name" :key="item.name">{{ item.name }}</Option>
                  </Select>
                </FormItem> -->
                <FormItem label="公司全称" prop="name">
                  <auto-complete clearable ref="text1" v-model="formItem.name" @on-change="changeSel" @on-select="selectSel" style="width:100%"  >
...
export default {
  ...
  methods: {
    getMyProject() {
      this.$http.post('/api/engine/project/listAll', { operator: this.$user.userId,step:'待入库',source:'入库申请'}).then((res) => {
        this.loading = 0;
        if (res.data.code == 0 ) {
    ...
    saveItem() {
      let url = '';
      let msg = '';
      let bool= false;//是否编辑，或者新加
      this.selectProject.map((it)=>{
        if(this.formItem.id==it.id&&this.formItem.name==it.name){
          bool = true;
        }
      })
      if (bool) {
