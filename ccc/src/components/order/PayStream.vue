<template>
  <ListPage
    ref="page"
    title="订单管理"
    api="/api/engine/order/payStreamList"
    :model="this"
    :beforeLoad="beforeLoad"
    @on-load="onLoadData"
  >
    <OrderTabs tabvalue="pay"></OrderTabs>
    <div class="page-searchbox">
      <table cellpadding="0" cellspacing="0">
        <tr>
          <td>
            <Input style="min-width: 100px" v-model="queryForm.orderId" placeholder="订单编号" @keyup.enter.native="query" />
          </td>
          <td>
            <Select
              clearable
...
export default {
  components: {
    ListPage, OrderTabs, Detail, Log, PayOrderDetail
  },
  data() {
    let that = this;
    return {
